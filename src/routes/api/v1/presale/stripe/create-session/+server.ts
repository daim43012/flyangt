...
export const POST = async ({ request, cookies, url }: RequestEvent) => {
  try {
    ...
    // successUrl can be overridden by client (for mobile deep link)
    const successUrl =
      body.successUrl ?? `${url.origin}/app/presale/success?session_id={CHECKOUT_SESSION_ID}`;
    const cancelUrl = body.cancelUrl ?? `${url.origin}/app/presale/cancel`;
    ...
    return json({ ok: false, message: e?.message || String(e) }, { status });
  }
};
